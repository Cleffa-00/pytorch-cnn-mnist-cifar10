...
Train a CNN model on MNIST dataset with data augmentation
...
from models.cnn import MNISTNet
...
def evaluate_per_class(model, testloader, device):
    ...
    for i in range(10):
        ...
        print(f"  Digit {CLASSES[i]}: {acc:5.1f}%")
    print("-" * 30)
...
def main():
    ...
    for epoch in range(args.epochs):
        ...
        if test_acc > best_acc:
            best_acc = test_acc
            save_path = os.path.join(args.save_dir, 'mnist_best.pth')
    ...
    print(f"  Model saved to: {args.save_dir}/mnist_best.pth")
    
    # Per-class accuracy
    model.load_state_dict(torch.load(os.path.join(args.save_dir, 'mnist_best.pth')))
